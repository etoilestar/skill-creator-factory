...
def _workflow_step_planner_prompt() -> str:
    ...
    return (
        ...
        "default binding 只输出 source_type，默认值由 Action Schema 注入；expected_outputs 如提供必须与"
        "Action Schema outputs 一致，Backend 最终以 Action Schema 为真值。step 和顶层不得增加 command、"
        "执行结果或其他协议外字段。"
...
async def _execute_runtime_plan(
    *, execution_root: Path, action_schema: dict, runtime_plan: dict,
    ...
    """Execute canonical steps and adapt only at checkpoints or runtime errors."""
    ...
    attempt_results, attempt_output_files = [], []
    ...
    def common_result() -> dict:
        return {
            "runtime_plan": plan, "results": results, "context": context,
            "runtime_instances": instances, "output_files": output_files,
            "accepted_output_files": output_files, "attempt_results": attempt_results,
            "attempt_output_files": attempt_output_files,
    ...
    while cursor < len(plan["steps"]):
        ...
        attempt_number = retry_counts.get(step["step_id"], 0) + 1
        items = [None]
        failure_result = None
        trigger = "checkpoint"
        try:
        ...
        observations, current_results, current_files, current_touched, current_instances = [], [], [], [], []
        if failure_result is None:
            for instance_index, item in enumerate(items):
                ...
                current_touched.extend(task_touched)
                current_files.extend(result.get("output_files") or [])
                ...
                instance = {"step_id": step["step_id"], "instance_index": instance_index,
                            "attempt": attempt_number, "accepted": False,
                            "success": bool(result.get("success", True)) and failure_result is None,
                            "output": observation}
        ...
        attempt_results.extend(current_results)
        attempt_output_files.extend(current_files)
        touched.extend(current_touched)
        instances.extend(current_instances)
        successful = failure_result is None
        reason = checkpoints.get(step["step_id"]) if successful else "runtime exception requires a model decision"
        should_decide = policy is not None and ((successful and reason is not None) or not successful)
        decision = {"action": "continue", "reason": "deterministic continuation"}

        if should_decide and adaptive_decision_count < MAX_ADAPTIVE_DECISIONS:
            observation = ((observations if step["foreach"] else observations[0]) if successful else {
            ...
            safe = _safe_observation_for_agent(step=step, observation=observation, context=context,
                remaining_steps=plan["steps"][cursor + 1:], output_files=[*output_files, *current_files])
            adaptive_decision_count += 1
            try:
                decision = await _decide_after_observation(
                    user_request=str(input_envelope.get("user_request") or ""), runtime_plan=plan,
                    completed_step_ids=[*completed_step_ids, *([step["step_id"]] if successful else [])],
                    safe_observation=safe, action_schema=action_schema, adaptive_reason=reason,
                    trigger=trigger, model=model,
                )
            except Exception as exc:
                logger.warning("adaptive decision unavailable at %s: %s", step["step_id"], exc)
                decision = {"action": "continue" if successful else "stop_failure",
                            "reason": "adaptive model unavailable", "fallback": True}
            action = decision["action"]
            trace_decision = (("fallback_continue" if successful else "fallback_stop_failure")
                              if decision.get("fallback") else action)
            adaptive_trace.append({
                "round": adaptive_decision_count, "after_step_id": step["step_id"], "trigger": trigger,
                "decision": trace_decision, "reason": decision.get("reason", ""),
                "plan_revision": revision_count, "retry_attempt": retry_counts.get(step["step_id"], 0),
            })
            if yield_func:
                await yield_func(_sse_react_event("adaptive_decision", decision.get("reason", ""), {
                    "step_id": step["step_id"], "action": trace_decision,
                }))
        elif not successful:
            decision = {"action": "stop_failure", "reason": "adaptive decision budget exhausted"}
        ...
        if action == "retry_current" and retry_counts.get(step["step_id"], 0) < MAX_RETRY_PER_STEP:
            ...
            if bindings is not None:
                ...
                raw_retry_plan = {**plan, "steps": [
                    {**item, "bindings": {**item["bindings"], **bindings}}
                    if item["step_id"] == step["step_id"] else item
                    for item in plan["steps"]
                ]}
                retry_plan = validate_runtime_execution_plan(raw_retry_plan, action_schema, input_envelope, resource_catalog)
                if retry_plan["missing_required_inputs"]:
                    raise RuntimePlanError("runtime_plan_missing_input", "retry bindings are unresolved",
        ...
        if successful:
            ...
            output_files.extend(current_files)
        ...
        if action == "replan_remaining" and revision_count < MAX_PLAN_REVISIONS:
            ...
            if plan["missing_required_inputs"]:
                return {**common_result(), "executed": True, "success": None, "completed": False,
                        "paused_for_user": True, "mode": "ask_user",
                        "reason": "revised plan requires additional input",
            ...
            if yield_func:
                await yield_func(_sse_react_event("runtime_plan_revised", "剩余计划已重新校验", {
                    "revision": revision_count,
                    "remaining_step_count": len(plan["steps"]) - len(completed_step_ids),
                    "adaptive_checkpoint_count": len(checkpoints),
                }))
            cursor = len(completed_step_ids)
            continue
        if action == "replan_remaining":
            return {**common_result(), "executed": False, "success": False,
                    "adaptive_limit_reached": "plan_revisions",
                    "failed_step_id": None if successful else step["step_id"],
        ...
        if not successful:
            if policy is None:
                output_files.extend(current_files)
            failure = {**common_result(), "executed": False, "success": False,
                       "failed_step_id": step["step_id"],
                       "failed_instance_index": len(observations),
    ...
    if yield_func:
        await yield_func(_sse_react_event("workflow_complete", "工作流执行完成", {"step_count": len(completed_step_ids)}))
        await yield_func(None)
    return {**common_result(), "executed": True, "success": True,
            "stopped_early": stopped_early, "stop_reason": "adaptive" if stopped_early else None}
...
async def _execute_skill_workflow(
    *,
    ...
    if planned_here:
        ...
        if not dataflow_plan.get("missing_required_inputs"):
            skill_path = execution_root / "SKILL.md"
