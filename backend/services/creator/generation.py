...
def _build_generate_file_prompt(
    ...
    if file_path == "SKILL.md":
        instruction = (
            ...
            "10. JSON argv 必须是标准 JSON；动态值必须作为 JSON 字符串值出现。__RUNTIME_INPUT_FILE__ / FILES 只表示平台上传文件；FILE / FILES 不能代替前序 stdout。\n"
            ...
            "19. 命令中 JSON key 由当前脚本真实接口决定；argv value 由 ResponsibilityEdge/source_kind 决定。没有 incoming edge 的可选参数才允许使用 literal/default。\n"
            "20. 不要把 FILE / FILES / __RUNTIME_INPUT_FILE__ 用作前序 stdout 的替代来源；下游脚本 incoming edge 必须绑定对应 from_output placeholder。\n"
