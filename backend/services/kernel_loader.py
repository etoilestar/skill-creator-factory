...
def _compose_creator_workflow_contract_for_phase(phase: str) -> str:
    ...
    if phase == "prepare_plan":
        return base_contract + (
            ...
            "3. needs_clarification 的 clarifying_questions 必须只包含 1 个问题；每轮只能问一个问题。"
            "该问题必须包含 2-4 个选项，并统一使用连续的 A.、B.、C.、D. 内联编号，"
            "格式必须为：问题正文？A. 选项1 B. 选项2 C. 选项3 D. 选项4。"
            "只输出实际需要的 2-4 个选项，编号必须从 A 连续开始。"
            "不要使用“选项：”后接 Markdown 列表、①②③④、(A)/(B) 或其他选项表现形式。\n"
            ...
            "7. 澄清问题的语义内容可以参考 Loaded SKILL.md 中的示例，"
            "但 clarifying_questions 的传输格式必须始终遵循上述 A./B./C./D. canonical format；"
            "不得照搬 Markdown bullet 形式。\n"
            ...
            "13. 如果 clarification_rounds 达到上限，不得继续返回业务澄清问题；必须归纳创建要点并询问用户是否补充。\n"
            "14. 创建要点必须体现蓝图和责任图谱合同需要落实的功能，不要输出风险项。\n"
            "15. 用户确认无补充后，不得继续 needs_clarification，必须生成 internal_blueprint_text。\n"
            "16. 用户补充后，重新归纳要点；达到补充上限后必须生成 internal_blueprint_text。\n\n"
            "输出边界：\n"
...
3. 完整蓝图之后，再输出一个 AskUserQuestion 确认问题；确认问题/选项/按钮状态属于 Creator UI 外层流程，不属于蓝图正文，不要把它们写入 SkillPlan、references 或 assets。
