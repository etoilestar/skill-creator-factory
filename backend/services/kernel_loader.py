...
def _compose_creator_workflow_contract_for_phase(phase: str) -> str:
    ...
    if phase == "first_time" or phase == "phase1":
        ...
        return base_contract + (
            ...
            "- 如果用户要求使用平台内置图像/多模态模型，不要生成外部 API key、关键词数据库或假图片脚本；应说明由宿主配置的模型能力完成相关步骤。创作类脚本必须调用宿主注入的 LLM_BASE_URL + TEXT_MODEL，图像/视觉步骤使用 IMAGE_MODEL/VISION_MODEL 或输出给宿主模型的提示词，禁止固定模板/随机词表/ASCII 图冒充模型生成。\n\n"
            "Phase 3+ 行为约束：\n"
...
2. **蓝图格式必须严格按照 SKILL.md 中的模板**，包括：
   ...
   - 如果涉及童话/故事/科普/文章/歌词等创作能力，优先设计为模型直接回答；若必须生成 scripts/，脚本必须调用宿主注入的 LLM_BASE_URL + TEXT_MODEL，不得用固定模板/随机词表/ASCII 图替代模型创作
