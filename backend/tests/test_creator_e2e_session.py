...
def _default_value_seed_case(tmp_path, *, external_context=None):
    ...
    command = E2EWorkflowCommand(
        ...
        "python scripts/x.py '{}'",
        "python",
        {"prompt": "{{user_request}}", "count": "{{fields.count}}"},
    )
    return e2e._seed_initial_e2e_payload(
        ...
        requirements_by_file={
            "scripts/x.py": [
                e2e.RequirementItem(
                    target_file="scripts/x.py", inputs=["prompt: string", "count: int"]
                )
            ]
        },
        skill_plan_entries={
