<template>
  <section class="multi-plan">
    ...
    <ol>
      <li v-for="step in normalizedSteps" :key="step.stepId">
        <div class="skill"><code>{{ step.stepId }}</code><strong>{{ step.skillName }}</strong></div>
        <p>{{ step.task || '完成用户任务' }}</p>
        <div v-if="step.dependsOn.length" class="flow">依赖：{{ step.dependsOn.join('、') }}</div>
        <div v-if="step.inputSources.length" class="flow">输入：{{ step.inputSources.join('；') }}</div>
      </li>
    </ol>
...
import { normalizeMultiSkillStepForUI } from '../../composables/useSandboxChat.js'
const props = defineProps({ plan: { type: Object, default: null }, confirming: Boolean, confirmable: Boolean })
defineEmits(['confirm', 'cancel'])
const normalizedSteps = computed(() => {
  const steps = props.plan?.preview?.length ? props.plan.preview : props.plan?.steps || []
  return steps.map(normalizeMultiSkillStepForUI)
})
</script>
<style scoped>
