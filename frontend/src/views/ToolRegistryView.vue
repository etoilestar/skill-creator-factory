...
function logAuthor(event) { authorLogs.value.push({ time: new Date().toLocaleTimeString(), ...event }); if (authorLogs.value.length > 80) authorLogs.value.shift(); if (event.message) statusMessage.value = event.message }
