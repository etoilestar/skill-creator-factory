...
- **需要脚本/命令**: assistant 必须在 Sandbox 当轮回复中输出显式 fenced code block（如 ```bash ... ```），宿主只执行当轮回复中出现的 block。
...
6. **Block 触发执行**：如果 Skill 需要脚本、命令或写文件，SKILL.md 必须明确要求 assistant 在运行时输出显式 fenced code block；宿主不会因为 SKILL.md 中出现 `scripts/...` 行内路径就自动执行。
7. **不要假装执行**：SKILL.md 必须要求 assistant 等待宿主 observation，再基于 stdout/stderr/输出文件回答用户。
