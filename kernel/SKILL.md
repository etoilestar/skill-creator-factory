...
当 `status=needs_clarification` 时，`clarifying_questions` 必须使用带选项的问题。每个问题 2～4 个选项，尽量包含推荐项。问题必须聚焦阻塞点，不要恢复长问卷。
...
每次返回 `needs_clarification` 时，`clarifying_questions` 的最后一个问题必须是：

“还有其他需要补充的要求吗？A. 没有，按上面的选择继续 B. 有，我补充说明”

如果 `clarifying_questions` 已经有 3 个问题，则第三个问题必须使用这个补充确认问题。如果只需要问 1～2 个业务问题，也必须额外追加这个补充确认问题，但总数不得超过 3 个。
