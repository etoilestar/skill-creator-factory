...
- path: `scripts/<name>.py`
  ...
  outputs: [按 role 列出 stdout JSON 字段：text、image_paths/images、pdf_path/docx_path/pptx_path/file_paths、parsed_text、search_results、rows 或 draft_id；文件产物路径只在这里表达]
...
- **需要脚本/命令**: assistant 必须在 Sandbox 当轮回复中输出标准 Markdown fenced code block（如 ```bash ... ```），宿主只执行当轮回复中出现的 block。第一步接收用户请求或提取参数；后续脚本命令必须接收前序脚本 stdout JSON 中对应字段，不能每一步都重复传入原始用户文本。
